# dados-atletas.// dados-atletas.js
...
class Atleta {
    ...
    // Método para calcular a média válida (remove menor e maior nota e faz a média)
    calculaMediaValida() {
        const notasOrdenadas = [...this.notas].sort((a, b) => a - b);
        notasOrdenadas.shift(); // remove menor
        notasOrdenadas.pop();   // remove maior
        const soma = notasOrdenadas.reduce((acc, val) => acc + val, 0);
